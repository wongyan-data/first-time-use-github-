...
unsigned char *get_err_msg(int state, struct terminal *term) /* term may be NULL */
{
	unsigned char *e;
	size_t sl;
...
#if defined(BEOS) || defined(HAIKU)
	e = cast_uchar strerror_alloc(state, term);
	if (*e && !strstr(cast_const_char e, "No Error")) goto have_error;
	mem_free(e);
#endif
	e = cast_uchar strerror_alloc(-state, term);
	if (*e) goto have_error;
	mem_free(e);
	goto unk;
have_error:
...
static unsigned char *get_stat_msg(struct status *stat, struct terminal *term)
{
	...
	return stracpy(get_text_translation(get_err_msg(stat->state, term), term));
}
...
void print_screen_status(struct session *ses)
{
	...
	if (ses->screen && ses->screen->f_data && ses->screen->f_data->title && ses->screen->f_data->title[0]) {
...
{
	unsigned char *t = get_err_msg(stat->state, ses->term);
...
void download_window_function(struct dialog_data *dlg)
{
	...
	} else m = stracpy(get_text_translation(get_err_msg(stat->state, term), term));
...
static void download_file_error(struct download *down, int err)
{
	...
	if (ses) {
		unsigned char *emsg = err ? strerror_alloc(err, ses->term) : stracpy(cast_uchar "Zero returned");
...
have_frag:
	...
	if (stat->state < 0) {
		if (down->decompress) {
			...
			if (err) goto det_abt;
			if (download_prealloc(down, len))
				goto det_abt;
			while (down->last_pos + (tcount)0 < len) {
				if (download_write(down, start + down->last_pos, len - down->last_pos)) goto det_abt;
			}
		}
		if (stat->state != S__OK) {
			unsigned char *t = get_err_msg(stat->state, get_download_ses(down)->term);
...
int create_download_file(struct session *ses, unsigned char *cwd, unsigned char *fi, int mode, off_t siz)
{
	...
	if (h == -1) {
		...
		if (!ses) goto x;
		msg = stracpy(file);
		msge = strerror_alloc(errn, ses->term);
...
static struct f_data *format_html(struct f_data_c *fd, struct object_request *rq, unsigned char *url, struct document_options *opt, int *cch)
{
	...
	if (f->rq->ce) {
		...
		if (jsint_get_source(fd, &start, &len)) f->uncacheable = 1;
...
unsigned long formatted_info(int type)
{
	...
	switch (type) {
		...
		case CI_LOCKED:
			...
			return i;
		default:
			internal_error("formatted_info: bad request");
	}
	return 0;
}
...
struct additional_file *request_additional_file(struct f_data *f, unsigned char *url_)
{
	...
	h = hash_string(url) % AF_HASH_SIZE;
	for (af = f->af->hash[h]; af; af = af->hash_fwdlink) {
...
static void copy_additional_files(struct additional_files **a)
{
	...
	(*a)->refcount--;
	afs = mem_calloc(sizeof(struct additional_files));
	afs->refcount = 1;
	init_list(afs->af);
	foreachback(struct additional_file, af, laf, (*a)->af) {
		struct additional_file *afc;
		unsigned h = hash_string(af->url) % AF_HASH_SIZE;
...
static int is_forced_download(struct object_request *rq)
{
	...
	if ((cd = parse_http_header(ce->head, cast_uchar "Content-Disposition", NULL))) {
		int r;
		unsigned char *s;
		if ((s = cast_uchar strchr(cast_const_char cd, ';'))) *s = 0;
		r = !casestrcmp(cd, cast_uchar "attachment");
		mem_free(cd);
		return r;
	}
	return 0;
