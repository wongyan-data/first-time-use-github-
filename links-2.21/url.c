...
static int test_qualified_name(unsigned char *host, unsigned char *hostname)
{
	unsigned char *c;
	if (!casestrcmp(host, hostname))
		return 1;
	c = cast_uchar strchr(cast_const_char hostname, '.');
	if (c) {
		*c = 0;
		if (!casestrcmp(host, hostname))
			return 1;
...
static int is_local_host(unsigned char *host)
{
	if (!*host)
		return 1;
	if (!casestrcmp(host, cast_uchar "localhost"))
		return 1;
#if defined(HAVE_GETHOSTNAME)
	{
		...
		n[0] = 0;
		EINTRLOOP(rs, gethostname(cast_char n, sizeof n));
		n[sizeof n - 1] = 0;
		if (!rs && strlen(cast_const_char n) < sizeof n - 1) {
...
#elif defined(HAVE_SYS_UTSNAME_H) && defined(HAVE_UNAME)
	{
		...
		if (rs >= 0) {
			if (test_qualified_name(host, cast_uchar name.nodename))
				return 1;
		}
	}
#endif
	...
	if (rel[0] == '/' && rel[1] == '/' && !data) {
		...
		if (!(s = cast_uchar strstr(cast_const_char base, "//"))) {
			if (!(s = cast_uchar strchr(cast_const_char base, ':'))) {
				bad_base:
				internal_error("bad base url: %s", base);
				return NULL;
			}
			s++;
		}
